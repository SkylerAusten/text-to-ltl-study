...
class NodeRepr:
    ...
    def expand(self, literals):
        ...
        missing_literals = [
            literal for literal in literals if literal not in vars_words
        ]
...
def spotTraceToNodeReprs(sr):
    ...
    if len(prefix_split) > 1:
        ...
        cycle_states = [
            NodeRepr(part) for part in cycled_content.split(";") if part.strip() != ""
        ]
        cycle_states.append(cycle_states[0])

    return {"prefix_states": states, "cycle_states": cycle_states}
