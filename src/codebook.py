...
class MisconceptionCode(Enum):
    ...
    Syntactic = (
        "RandomSyntactic"  ### Should this be here??? Does adding this break anything?
    )
...
def getAllApplicableMisconceptions(node):
    ...
    xs = [
        applyMisconception(node, misconception) for misconception in MisconceptionCode
    ]
...
def applyTilFirst(node, f):
    ...
    if isinstance(node, BinaryOperatorNode):
        ...
        choose_left = res_left.misconception and (
            not res_right.misconception or random_index == 0
        )
        choose_right = res_right.misconception and (
            not res_left.misconception or random_index == 1
        )
...
def applyExclusiveU(node):
    if isinstance(node, BinaryOperatorNode) and node.operator == UntilNode.symbol:
        ...
        if isinstance(rhs, BinaryOperatorNode) and rhs.operator == AndNode.symbol:
            ...
            if (
                isinstance(rhs.left, UnaryOperatorNode)
                and rhs.left.operator == NotNode.symbol
                and LTLNode.equiv(rhs.left.operand, x)
            ):
    ...
    if isinstance(node, UntilNode):
        ...
        if isinstance(rhs, GloballyNode) and LTLNode.equiv(lhs, rhs.operand):
            return MutationResult(
                AndNode(lhs, FinallyNode(rhs)), MisconceptionCode.OtherImplicit
            )
    ...
    if isinstance(node, AndNode):
        ...
        if isinstance(lhs, FinallyNode) and isinstance(rhs, GloballyNode):
            ...
            if isinstance(rhs_operand, ImpliesNode):
                ...
                if (
                    LTLNode.equiv(lhs.operand, rhs_operand_lhs)
                    and isinstance(rhs_operand_rhs, NextNode)
                    and isinstance(rhs_operand_rhs.operand, GloballyNode)
                    and LTLNode.equiv(rhs_operand_lhs, rhs_operand_rhs.operand.operand)
                ):
                    return MutationResult(
                        AndNode(lhs.operand, NextNode(rhs)),
                        MisconceptionCode.OtherImplicit,
                    )

    if isinstance(node, NextNode):
        return MutationResult(
            FinallyNode(node.operand), MisconceptionCode.OtherImplicit
        )
...
def applyBadStateQuantification(node):
    ...
    if isinstance(node, FinallyNode):
        op = node.operand
        return MutationResult(
            GloballyNode(op), MisconceptionCode.BadStateQuantification
        )
