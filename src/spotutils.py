...
def doIntersect(formula1, formula2):
    spot.formula.intersects()
...
def generate_unique_traces(f_accepted: str, f_rejected: list, max_traces=5):
    """
    Generate unique traces accepted by f_accepted but not satisfying any of the
    provided rejected formulas.

    :param f_accepted: LTL formula string that defines accepted traces.
    :param f_rejected: List of LTL formula strings to reject.
    :param max_traces: Maximum number of traces to generate.
    :return: List of generated traces.
    """
    # Parse the accepted formula
    f_a = spot.formula(f_accepted)

    # Combine all rejected formulas using logical OR
    combined_rejected = spot.formula.Or([spot.formula(f) for f in f_rejected])

    # Negate the combined rejected formulas
    f_r = spot.formula.Not(combined_rejected)

    # Combine the accepted formula and the negated rejected formulas
    f = spot.formula.And([f_a, f_r])
    automaton = f.translate()

    # Generate traces that satisfy the automaton
    runs = generate_accepting_words(automaton, max_traces)
    return runs
