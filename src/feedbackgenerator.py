...
class FeedbackGenerator:
    ...
    def getCEWords(self):
        if self.disjoint():
            ...
            return spotutils.generate_traces(
                f_accepted=self.correct_answer, f_rejected=self.student_selection
            )
        elif self.correctAnswerContained():
            return spotutils.generate_traces(
                f_accepted=self.student_selection, f_rejected=self.correct_answer
            )
