...
def get_db_uri():
    ...
    if "DATABASE_URL" in os.environ:
        ...
        print(
            f"Using HEROKU PROVIDED database at {db_uri}, which was generated by replacing uri at {os.environ['DATABASE_URL']}"
        )
        return db_uri
    else:
...
class Logger:
    ...
    def getUserLogs(self, userId, lookback_days=30):
        ...
        with self.Session() as session:
            lookback_date = datetime.datetime.now() - datetime.timedelta(
                days=lookback_days
            )
            logs = (
                ...
                .filter(
                    ...
                    StudentResponse.timestamp >= lookback_date,
                )
                .all()
            )
            return logs
    ...
    def getComplexity(self, userId):
        ...
        with self.Session() as session:
            complexity = (
                session.query(GeneratedExercise.complexity)
                .filter(GeneratedExercise.user_id == userId)
                .order_by(GeneratedExercise.timestamp.desc())
                .first()
            )
    ...
    def getUserExercises(self, userId, lookback_days=30):
        ...
        with self.Session() as session:
            ...
            logs = (
                session.query(GeneratedExercise)
                .filter(
                    GeneratedExercise.user_id == userId,
                    GeneratedExercise.timestamp >= lookback_date,
                )
                .all()
            )
            return logs
    ...
    def getCourseResponses(self, course_name):
        ...
        with self.Session() as session:
            student_responses = (
                session.query(StudentResponse)
                .filter(StudentResponse.course == course_name)
                .all()
            )
