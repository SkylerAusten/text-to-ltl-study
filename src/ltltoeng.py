...
def recurrence_pattern_to_english(node):
    if type(node) is ltlnode.GloballyNode:
        ...
        if type(op) is ltlnode.FinallyNode:
            ...
            return (
                "there will always be a point in the future where "
                + op.operand.__to_english__()
            )
    return None
    ...
    if type(node) is ltlnode.GloballyNode:
        ...
        if type(op) is ltlnode.ImpliesNode:
            ...
            if type(right) is ltlnode.AndNode:
                ...
                rhs = right.right
                if (
                    type(lhs) is ltlnode.FinallyNode
                    and type(rhs) is ltlnode.FinallyNode
                ):
                    return (
                        "whenever "
                        + left.__to_english__()
                        + ", eventually"
                        + lhs.operand.__to_english__()
                        + " and "
                        + rhs.operand.__to_english__()
                    )
    return None
    ...
    if type(node) is ltlnode.FinallyNode:
        ...
        if type(op) is ltlnode.GloballyNode:
            ...
            if type(negated) is ltlnode.NotNode:
                return (
                    "eventually, it will never be the case that "
                    + negated.operand.__to_english__()
                )
    return None
...
def finally_globally_pattern_to_english(node):
    if type(node) is ltlnode.FinallyNode:
        ...
        if type(op) is ltlnode.GloballyNode:
            return (
                "eventually, it will always be the case that "
                + op.operand.__to_english__()
            )
    return None
    ...
    if type(node) is ltlnode.FinallyNode:
        ...
        if type(op) is ltlnode.AndNode:
            return (
                ...
                + op.left.__to_english__()
                + " and "
                + op.right.__to_english__()
            )
    return None
    ...
    if type(node) is ltlnode.UntilNode:
        ...
        if type(left) is ltlnode.UntilNode:
            return (
                ...
                + left.left.__to_english__()
                + " until "
                + left.right.__to_english__()
                + ", and this will continue until "
                + right.__to_english__()
            )
    return None
