...
from ltlListener import ltlListener, LTLErrorListener
...
class BinaryOperatorNode(LTLNode):
    ...
    def __electrum__(self):
        return (
            f"({self.left.__electrum__()} {self.operator} {self.right.__electrum__()})"
        )
...
class ImpliesNode(BinaryOperatorNode):
    ...
    def __to_english__(self, depth=0):
        ...
        # Potential patterns:
        patterns = [f"if {lhs}, then {rhs}", f"{rhs} is necessary for {lhs}"]
...
def check_ltl_string(s):
    # Create an input stream from the string
    input_stream = InputStream(s)

    # Create a lexer and attach the custom error listener
    lexer = ltlLexer(input_stream)
    error_listener = LTLErrorListener()
    ...
    # Create a parser and attach the custom error listener
    parser = ltlParser(token_stream)
    ...
    try:
        ...
        # Handle exceptions raised during parsing
        error_listener.errors.append(str(e))

    # Check for syntax errors
    if error_listener.get_errors():
        return None, error_listener.get_errors()
