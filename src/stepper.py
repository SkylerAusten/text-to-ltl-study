...
def randid():
    return "".join(
        random.choices("abcfghijklmopqrstuvwxyzABCFGHIJKLMOPQRSTUVWXYZ", k=6)
    )
...
class TraceSatisfactionResult:
    def __init__(
        self, prefix_states: list[StepperNode], cycle_states: list[StepperNode]
    ):
        self.prefix_states = prefix_states
        self.cycle_states = cycle_states

    def to_dict(self):
        return {"prefix_states": self.prefix_states, "cycle_states": self.cycle_states}
...
def satisfiesTrace(node, trace, traceindex, originaltrace, syntax) -> StepperNode:
    ...
    return StepperNode(
        formula, [], False, trace, traceindex=traceindex, originaltrace=originaltrace
    )


def buildNodeStep(
    node, subtrace, trace_index_of_subtrace, trace, syntax
) -> StepperNode:
    stepperNode = satisfiesTrace(
        node=node,
        trace=subtrace,
        traceindex=trace_index_of_subtrace,
        originaltrace=trace,
        syntax=syntax,
    )
...
def splitTraceAtCycle(sr):
    ...
    prefix_states = [
        x.strip() for x in prefix_split[0].strip().split(";") if x.strip() != ""
    ]
...
def traceSatisfactionPerStep(node, trace, syntax):
    ...
    prefix_sat = [
        buildNodeStep(
            node=node,
            subtrace=buildTraceForStateInPrefix(i),
            trace=trace,
            trace_index_of_subtrace=i,
            syntax=syntax,
        )
    ...
    cycle_sat = [
        buildNodeStep(
            node=node,
            subtrace=buildTraceForStateInCycle(i),
            trace=trace,
            trace_index_of_subtrace=num_prefix_states + i,
            syntax=syntax,
        )
